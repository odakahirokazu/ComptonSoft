...
AllSkyPrimaryGenerator::AllSkyPrimaryGenerator()
...
{
  add_alias("AllSkyPrimaryGenerator");
}

AllSkyPrimaryGenerator::~AllSkyPrimaryGenerator() = default;

ANLStatus AllSkyPrimaryGenerator::mod_define()
{
...
ANLStatus AllSkyPrimaryGenerator::mod_initialize()
{
  ...
  std::cout << "AllSkyPrimaryGenerator::mod_initialize \n"
            << "  FITS keys"
            << "    MAPMODE: " << map_mode_ << "\n"
...
#ifdef __ALLSKYPRIMARYGENERATOR_ENABLE_FUTURE_MAP_MODE__
...
anlgeant4::PrimarySetting AllSkyPrimaryGenerator::make_primary_setting() const
{
  using std::cos;
  using std::sin;
  using std::sqrt;
  ...
  const G4ThreeVector position = center_position() + v + v2;
...
ANLStatus AllSkyPrimaryGenerator::mod_end_run()
{
  ...
  std::cout << "AllSkyPrimaryGenerator::mod_end_run \n"
...
void AllSkyPrimaryGenerator::loadMultiBandImages(fitshandle* fits, int num_maps, ANLStatus& status)
{
  using anlgeant4::constant::pi;
...
void AllSkyPrimaryGenerator::constructMaps(ANLStatus& status)
{
  if (map_mode_ == "multiband") {
...
void AllSkyPrimaryGenerator::constructMapsMultiBand(ANLStatus& status)
{
  using std::log;
  using std::pow;
...
void AllSkyPrimaryGenerator::calculateMapIntegrals(ANLStatus& status)
{
...
int AllSkyPrimaryGenerator::sampleBandIndex() const
{
...
int AllSkyPrimaryGenerator::samplePixel(int band_index) const
{
  const double r = G4UniformRand();
...
void AllSkyPrimaryGenerator::setCoordinate(ANLStatus& status)
{
  using std::sin;
  using std::cos;
  using std::acos;
