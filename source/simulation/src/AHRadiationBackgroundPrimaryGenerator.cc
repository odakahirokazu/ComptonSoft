...
AHRadiationBackgroundPrimaryGenerator::~AHRadiationBackgroundPrimaryGenerator() = default;

ANLStatus AHRadiationBackgroundPrimaryGenerator::mod_define()
{
...
ANLStatus AHRadiationBackgroundPrimaryGenerator::mod_initialize()
{
  ...
  const double rate = integralParticleIntensity * circleArea * (4.0*CLHEP::pi*covering_factor());

  set_intensity(integralEnergyIntensity);
