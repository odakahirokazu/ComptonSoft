...
ANLStatus CalcPedestal_SpW::mod_his()
{
  VCSModule::mod_his();
  ...
  char title[256];
  const int NBIN = 4096;
  const double ADCMIN = -2048.5;
  const double ADCMAX = +2047.5;
  ...
  std::vector<OneASICData*>::iterator itChipEnd = (*itDet)->ASICDataEnd();
    ...
    while ( itChip != itChipEnd ) {
      ...
      TH1* hisPed = new TH1D(name, title, nCh, -0.5, static_cast<double>(nCh)-0.5);
	...
	sprintf(title, "spectrum_%02d_%d_%02d", (*itDet)->getID(), j, k);
        TH1* hisSpec = new TH1I(name, title, NBIN, ADCMIN, ADCMAX);
...
ANLStatus CalcPedestal_SpW::mod_ana()
{
...
ANLStatus CalcPedestal_SpW::mod_endrun()
{
  const int ADCZeroLevelBin = 2048;
  const int SearchHalfWidth = 1024;
