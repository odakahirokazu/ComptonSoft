...
AEObservationPrimaryGenerator::AEObservationPrimaryGenerator()
  : arfFileName_("source.arf")
{
  add_alias("AEObservationPrimaryGenerator");
}

ANLStatus AEObservationPrimaryGenerator::mod_define()
{
  anlnext::ANLStatus status = BasicPrimaryGenerator::mod_define();
  if (status!=AS_OK) {
    return status;
  }
...
ANLStatus AEObservationPrimaryGenerator::mod_pre_initialize()
{
  ANLStatus status = BasicPrimaryGenerator::mod_pre_initialize();
  if (status!=AS_OK) {
    return status;
  }
...
ANLStatus AEObservationPrimaryGenerator::mod_initialize()
{
  ANLStatus status = BasicPrimaryGenerator::mod_initialize();
  if (status!=AS_OK) {
    return status;
  }
...
ANLStatus AEObservationPrimaryGenerator::mod_analyze()
{
  if (sumFlux_ > flux_) {
    return AS_QUIT;
  }
  ...
  return BasicPrimaryGenerator::mod_analyze();
}

PrimarySetting AEObservationPrimaryGenerator::make_primary_setting() const
{
...
void AEObservationPrimaryGenerator::buildPositionIntegral()
{
...
G4ThreeVector AEObservationPrimaryGenerator::sample_position() const
{
...
double AEObservationPrimaryGenerator::overlap(double v1, double v2, double w1, double w2)
{
  if (v1>v2 || w1>w2){
    return 0.0;
  }
