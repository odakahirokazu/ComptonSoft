...
AHRayTracingPrimaryGenerator::AHRayTracingPrimaryGenerator()
  : m_FileName("raytracing.fits"),
    m_EventNum(0),
    m_ID(0)
{
  add_alias("AHRayTracingPrimaryGenerator");
}

ANLStatus AHRayTracingPrimaryGenerator::mod_define()
{
...
ANLStatus AHRayTracingPrimaryGenerator::mod_initialize()
{
...
ANLStatus AHRayTracingPrimaryGenerator::mod_analyze()
{
  if (m_ID == m_EventNum) {
    return AS_QUIT;
  }
...
PrimarySetting AHRayTracingPrimaryGenerator::make_primary_setting() const
{
  const int id = m_ID;
  ...
  const G4ThreeVector direction = G4ThreeVector(xDirection, yDirection, zDirection).unit();
  if (m_EnergyResample) {
