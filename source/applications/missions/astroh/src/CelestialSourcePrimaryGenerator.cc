...
CelestialSourcePrimaryGenerator::CelestialSourcePrimaryGenerator()
  : fitsFilenameI_("imageI.fits"),
    fitsFilenameQ_("imageQ.fits"),
    fitsFilenameU_("imageU.fits")
{
  add_alias("CelestialSourcePrimaryGenerator");
}

CelestialSourcePrimaryGenerator::~CelestialSourcePrimaryGenerator() = default;

ANLStatus CelestialSourcePrimaryGenerator::mod_define()
{
  ANLStatus status = IsotropicPrimaryGenerator::mod_define();
  if (status != AS_OK) {
    return status;
  }
...
ANLStatus CelestialSourcePrimaryGenerator::mod_initialize()
{
  ANLStatus status = IsotropicPrimaryGenerator::mod_initialize();
  if (status != AS_OK) {
    return status;
  }
...
anlgeant4::PrimarySetting CelestialSourcePrimaryGenerator::make_primary_setting() const
{
  using std::cos;
  using std::sin;
  using std::sqrt;
  ...
  G4ThreeVector centerDirection = center_direction();
  const G4ThreeVector centerPosition = center_position();
  ...
  v2.setMag( r * sqrt(G4UniformRand()) );
  ...
  if (!setPolarization_) {
    return make_primary(position, energy, direction);
  }
  ...
  if (G4UniformRand() < polDegree) {
    return make_primary(position, energy, direction, polarizationVector);
  }
  else {
...
ANLStatus CelestialSourcePrimaryGenerator::mod_end_run()
{
  const double r = radius();
  const double area = constant::pi*r*r;
  ...
  std::cout << "CelestialSourcePrimaryGenerator::mod_end_run \n"
            << "  Number: " << number() << "\n"
...
void CelestialSourcePrimaryGenerator::inputImage(std::string filename, image_t& image, ANLStatus* status)
{
...
void CelestialSourcePrimaryGenerator::makePolarizationMap(ANLStatus* status)
{
...
void CelestialSourcePrimaryGenerator::setCoordinate(ANLStatus* status)
{
  using std::sin;
  using std::cos;
...
void CelestialSourcePrimaryGenerator::buildPixelIntegral()
{
...
std::pair<int, int> CelestialSourcePrimaryGenerator::samplePixel() const
{
  std::pair<int, int> p;
