...
bool MTNNEventReconstructionAlgorithm::loadParameters(boost::property_tree::ptree& pt)
{
    ...
    //Setup NN models
    string model_dir = pt.get<string>("model_dir");

    vector<string> model_filenames;
    for(const auto& r: pt.get_child("model_filenames")){
        model_filenames.push_back(r.second.get_value<string>());
        std::cout << r.second.get_value<string>() << std::endl;
    }
    ...
    for(int ihit=min_hits_;ihit<=max_hits_;ihit++){
        model_paths_[ihit - min_hits_] = model_dir + string("/") + model_filenames[ihit - min_hits_];
    }
...
void MTNNEventReconstructionAlgorithm::setupModel(vector<string> model_paths)
{
