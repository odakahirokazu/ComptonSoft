...
NNAnalyzer::NNAnalyzer(const char* trained_model_path, int model_num_hits)
{
    ...
    trained_model_path_str_ = string(trained_model_path);
                ...
                BasicComptonEvent_sptr& eventReconstructed)
{
    ...
    if((escape_flag_==true) || (distinguish_escape_==false)){
        ...
        float hit3_energy_tmp = -0.5 * hit2_energy_tmp_ 
                            ...
                            + MASS_ELECTRON_KEV * unit::keV * hit2_energy_tmp_ 
...
{
    vector<int> initial(num_hits);
    std::map< vector<int>, int> tuple_to_label;
    std::map< int, vector<int> > label_to_tuple;
    ...
    do {
        vector<int> first_part(initial.begin(), initial.begin() + first_n_hits_);
