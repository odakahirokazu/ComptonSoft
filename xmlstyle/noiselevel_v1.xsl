...
<xsl:template match="noiselevel">
  <p>Detector: <xsl:value-of select="name" /></p>
  <p>Comment: <xsl:value-of select="comment" /></p>
  <xsl:apply-templates select="detectors/detector" />
  <hr />
</xsl:template>

<xsl:template match="detector">
  <h2>Detector ID: <xsl:value-of select="./@id" /></h2>
  <xsl:apply-templates select="chip" />
</xsl:template>

<xsl:template match="chip">
  <h3>Chip ID: <xsl:value-of select="./@id" /></h3>
  <table>
    <thead>
      <tr>
        <th>Channel</th>
