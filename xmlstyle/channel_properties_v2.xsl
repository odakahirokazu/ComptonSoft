...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns="http://www.w3.org/1999/xhtml">
  ...
  <xsl:template match="channel_properties">
  ...
  <xsl:template match="section|frame">
    ...
    <xsl:variable name="com_pedestal_value" select="common/pedestal/@value" />
    <xsl:variable name="com_gain_c0" select="common/gain_correction/@c0" />
    <xsl:variable name="com_gain_c1" select="common/gain_correction/@c1" />
    <xsl:variable name="com_gain_c2" select="common/gain_correction/@c2" />
    <xsl:variable name="com_gain_c3" select="common/gain_correction/@c3" />
    ...
    <tr class="common_param">
      ...
      <td><xsl:value-of select="$com_pedestal_value"/></td>
      <td><xsl:value-of select="$com_gain_c0"/></td>
      <td><xsl:value-of select="$com_gain_c1"/></td>
      <td><xsl:value-of select="$com_gain_c2"/></td>
      <td><xsl:value-of select="$com_gain_c3"/></td>
    ...
    <xsl:apply-templates select="channel">
      ...
      <xsl:with-param name="com_pedestal_value" select="$com_pedestal_value" />
      <xsl:with-param name="com_gain_c0" select="$com_gain_c0" />
      <xsl:with-param name="com_gain_c1" select="$com_gain_c1" />
      <xsl:with-param name="com_gain_c2" select="$com_gain_c2" />
      <xsl:with-param name="com_gain_c3" select="$com_gain_c3" />
      <xsl:with-param name="com_threshold" select="$com_threshold" />
    </xsl:apply-templates>
  </xsl:template>
  <xsl:template match="channel|pixel">
    <xsl:param name="type" select="''" />
    <xsl:param name="id" select="''" />
    ...
    <xsl:param name="com_pedestal_value" select="''" />
    <xsl:param name="com_gain_c0" select="''" />
    <xsl:param name="com_gain_c1" select="''" />
    <xsl:param name="com_gain_c2" select="''" />
    <xsl:param name="com_gain_c3" select="''" />
    <xsl:param name="com_threshold" select="''" />
    <xsl:variable name="channel" select="./@id" />
    <xsl:variable name="pixel_x" select="./@x" />
    <xsl:variable name="pixel_y" select="./@y" />
    ...
    <xsl:variable name="pedestal_value" select="pedestal/@value" />
    <xsl:variable name="gain_c0" select="gain_correction/@c0" />
    <xsl:variable name="gain_c1" select="gain_correction/@c1" />
    <xsl:variable name="gain_c2" select="gain_correction/@c2" />
    <xsl:variable name="gain_c3" select="gain_correction/@c3" />
    ...
    <tr>
      ...
      <td><xsl:value-of select="$pixel_x" /></td>
      <td><xsl:value-of select="$pixel_y" /></td>
      <td>
      ...
      <td>
        <xsl:call-template name="display_parameter" >
          <xsl:with-param name="common" select="$com_pedestal_value" />
          <xsl:with-param name="special" select="pedestal_value" />
        </xsl:call-template>
      </td>
      <td>
        <xsl:call-template name="display_parameter" >
          <xsl:with-param name="common" select="$com_gain_c0" />
          <xsl:with-param name="special" select="gain_c0" />
        </xsl:call-template>
      </td>
      <td>
        <xsl:call-template name="display_parameter" >
          <xsl:with-param name="common" select="$com_gain_c1" />
          <xsl:with-param name="special" select="gain_c1" />
        </xsl:call-template>
      </td>
      <td>
        <xsl:call-template name="display_parameter" >
          <xsl:with-param name="common" select="$com_gain_c2" />
          <xsl:with-param name="special" select="gain_c2" />
        </xsl:call-template>
      </td>
      <td>
        <xsl:call-template name="display_parameter" >
          <xsl:with-param name="common" select="$com_gain_c3" />
          <xsl:with-param name="special" select="gain_c3" />
        </xsl:call-template>
      </td>
      <td>
        <xsl:call-template name="display_parameter" >
