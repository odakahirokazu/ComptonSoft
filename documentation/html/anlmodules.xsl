...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns="http://www.w3.org/1999/xhtml">
  ...
  <xsl:template match="/">
	...
	<title>Compton Soft/ANL Modules</title>
      </head>
      
      <body>
	<div>&gt; <a href="index.html">Compton Soft Home</a> &gt; ANL Modules</div>
  ...
  <xsl:template match="param">
    <tr>
      <xsl:if test="string-length(@map_type)&gt;0">
	<xsl:attribute name="class">map_type_<xsl:value-of select="@map_type" /></xsl:attribute>
      </xsl:if>
      <td><xsl:value-of select="name" /></td>
      <td><xsl:value-of select="type" /><xsl:if test="string-length(@map_type)&gt;0"> [<xsl:value-of select="@map_type" />]</xsl:if>
      </td>
