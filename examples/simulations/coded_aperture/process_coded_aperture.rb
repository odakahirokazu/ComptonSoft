...
class DecodeImage < ANL::ANLApp
  ...
  def setup()
    ...
    with_parameters(num_pixels_x: @pixelx,
                    num_pixels_y: @pixely)
    chain :ReadXrayEventTree
    with_parameters(file_list: @inputs)
    chain :ExtractXrayEventImage, "ExtractXrayEventImage"
    with_parameters(num_x: @pixelx,
                    num_y: @pixely,
                    image_center_x: 496.0,
                    image_center_y: 496.0,
                    rotation_angle: 0.0,
                    new_origin_x: 0.0,
                    new_origin_y: 0.0)
    chain :ProcessCodedAperture, "ProcessCodedAperture"
    with_parameters(aperture_num_x: 31,
                    aperture_num_y: 31,
                    sky_num_x: 101,
                    sky_num_y: 101,
                    detector_element_size: 2.5E-4,
                    aperture_element_size: 80.0E-4,
                    detector_to_aperture_distance: 22.3,
                    sky_fov_x: 1000.0*arcsecond,
                    sky_fov_y: 1000.0*arcsecond,
                    decoding_mode: 1,
                    num_decoding_iterations: 1,
